# zfg 第二部分（模型五）：raw eeg -> eeg to spec
'''
模型五：模型五：eeg to spec（512，512）（数据增强），训练EfficientNet_b4模型
...
class CFG:
    model_name = '../efficientnetb4'  # 'google/efficientnet-b4'  # 使用EfficientNet-B4
    pretrained = True  # 使用预训练权重
    num_classes = 6  # 输出类别为6
...
all_eeg2spec = np.load('../output_3_4_5/combine_115.npy', allow_pickle=True).item()
...
class HMSHBACSpecModel(nn.Module):
    def __init__(self, model_name: str, pretrained: bool, in_channels: int, num_classes: int):
        ...
        # 如果输入通道数不是3，则修改第一层卷积层的输入通道数
        if in_channels != 3:
            self.model.efficientnet.embeddings.convolution = nn.Conv2d(
                in_channels,
                self.model.efficientnet.embeddings.convolution.out_channels,
                kernel_size=self.model.efficientnet.embeddings.convolution.kernel_size,
                stride=self.model.efficientnet.embeddings.convolution.stride,
                padding=self.model.efficientnet.embeddings.convolution.padding,
                bias=False
            )
