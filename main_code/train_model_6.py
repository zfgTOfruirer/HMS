...
'''
模型六：50s与10s 拼接的 eeg to spec 频谱数据（512，512），训练EfficientNet_v2模型
'''
import os
import random
import shutil
...
TRAIN_SPEC = DATA / "train_spectrograms_small_6"
TEST_SPEC = DATA / "test_spectrograms_small_6"
...
# train['total_votes'] = train[['seizure_vote', 'lpd_vote', 'gpd_vote', 'lrda_vote', 'grda_vote', 'other_vote']].sum(
#     axis=1)
train['target'] = train['expert_consensus']
train[CLASSES] /= train[CLASSES].sum(axis=1).values[:, None]
print((train['total_votes'] < 10).sum())  # 小于10的行数
print((train['total_votes'] >= 10).sum())  # 大于或等于10的行数
...
all_eeg2spec_2 = np.load('../hms-harmful-brain-activity-classification/output_6/combine_115.npy',
...
best_log_list = []
for (fold_id, best_epoch, _) in score_list:
    exp_dir_path = Path(OUTPUT / f"stage2/fold{fold_id}")
    best_model_path = exp_dir_path / f"snapshot_epoch_{best_epoch}.pth"
    copy_to = OUTPUT / f"stage2/best_model_fold{fold_id}.pth"
    shutil.copy(best_model_path, copy_to)
